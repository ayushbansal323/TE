...
section .data
	...
	ano		db 10,"ML assignment 07 :- Bubble sort using file operations"
			db 10,"---------------------------------------------------",10
	ano_len	equ	$-ano
...
_start:
		...
		dec	rax					; EOF
		mov	[abuf_len],rax                  ;total numbers in abuf_len
...
iloop:	mov	rdi,rsi		; a[j+1]
		...
		mov	al,[rsi]      ;value inside rsi moved in al
		cmp	al,[rdi]
		jbe	next
...
next:		inc	rsi
	...
	fwrite	[filehandle],array,[n]
        ...
        fwrite  [filehandle],exitmsg, exitmsg_len
	fclose [filehandle]	
...
Error1:
	...
	RET
;------------------------------------------------------------------
buf_array:                            ;moving elements in array with counter [n]
	xor	rcx,rcx
	xor	rsi,rsi
	xor	rdi,rdi
