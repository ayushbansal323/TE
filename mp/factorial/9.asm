
%macro cmn 4			;common macro for input/output
	mov rax,%1
	mov rdi,%2
	mov rsi,%3
	mov rdx,%4
	syscall
%endmacro

section .data
	num db 00h
	msg db "Factorial is : "
	msglen equ $-msg
	msg1 db "*****Program to find Factorial of a number***** ",0Ah
		db "Enter the number : ",
	msg1len equ $-msg1
	msgth db 0Ah,"Thank you for using program ",0Ah
	msgthlen equ $-msgth
	zerofact db " 00000001 "
	zerofactlen equ $-zerofact

section .bss
	dispnum resb 6
	result resb 4
	temp resb 3
	

section .text
global _start
_start:
	
	cmn 1,1,msg1,msg1len
	cmn 0,0,temp,3			;accept number from user
	call convert			;convert number from ascii to hex
	mov [num],dl
	
	cmn 1,1,msg,msglen
	
	xor rdx,rdx
	xor rax,rax
	mov al,[num]			;store number in accumulator
	cmp al,01h			
	jbe endfact
	xor rbx,rbx
	mov bl,01h
	call factr			;call factorial procedure
	call display

	call exit
endfact:
	cmn 1,1,zerofact,zerofactlen
	call exit

	factr:				;recursive procedure
	
			cmp rax,01h
			je retcon1
			push rax			;put (rbx-1) in stack
			dec rax    ;decrement rax to put it in stack
			
			call factr  ;calculate facto for (rbx-1)

		retcon:
			pop rbx      ;retrieve rbx from stack
			mul rbx      ;
			jmp endpr

		retcon1:			;if rax=1 return
			pop rbx   ;retrieve rbx from stack
			jmp retcon		
		endpr:
	
	ret

	display:			; procedure to convert hex to ascii
	
			mov rsi,dispnum+3
			xor rcx,rcx
			mov cl,4

		cont:
			xor rdx,rdx
			xor rbx,rbx
			mov bl,10h
			div ebx
			cmp dl,09h
			jbe skip
			add dl,07h
		skip:
			add dl,30h
			mov [rsi],dl
			dec rsi
			loop cont
	
			cmn 1,1,dispnum,4
	
	ret

	convert:			;procedure to convert ascii to hex
			mov rsi,temp
			mov cl,02h
			xor rax,rax
			xor rdx,rdx
		contc:
			rol dl,04h
			mov al,[rsi]
			cmp al,39h
			jbe skipc
			sub al,07h
		skipc:
			sub al,30h
			add dl,al
			inc rsi
			dec cl
			jnz contc
	
	ret

	exit:				;exit system call
			cmn 1,1,msgth,msgthlen	
			mov rax,60
			mov rdi,0
			syscall

	ret
	
	
;	-------------------------------------------------------------------------------------------------
	
;	OUTPUT:-
	
;	ubuntu@ubuntu-ThinkCentre-A54:~$ nasm -f elf64 factorial.asm

 ;       ubuntu@ubuntu-ThinkCentre-A54:~$ ld -o factorial factorial.o

  ;      ubuntu@ubuntu-ThinkCentre-A54:~$ ./factorial

   ;     *****Program to find Factorial of a number***** 

    ;    Enter the number : 05

     ;   Factorial is : 0078

      ;  Thank you for using program
